...
TEST_CASE("flx: subscriptions persist after closing/reopening", "[sync][flx][app]") {
    ...
    TestAppSession test_session(harness.app_session());
    SyncTestFile config(test_session.app()->current_user(), harness.schema(), SyncConfig::FLXSyncEnabled{});
...
TEST_CASE("flx: connect to FLX as PBS returns an error", "[sync][flx][app]") {
    ...
    TestAppSession test_session(harness.app_session());
    SyncTestFile config(test_session.app(), bson::Bson{}, harness.schema());
