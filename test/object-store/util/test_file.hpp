...
struct HookedTransport : realm::app::GenericNetworkTransport {
    HookedTransport(realm::util::UniqueFunction<void(const realm::app::Response&)>* network_callback)
...
public:
    TestAppSession();
    TestAppSession(realm::AppSession, std::shared_ptr<realm::app::GenericNetworkTransport> = nullptr, DeleteApp = true);
    ~TestAppSession();
...
private:
    ...
    std::shared_ptr<realm::app::GenericNetworkTransport> m_transport = std::make_shared<HookedTransport>(&network_callback);
};
#endif
...
public:
    ...
    std::shared_ptr<realm::app::App> app() const noexcept { return m_app; }
    std::string base_file_path() const { return m_base_file_path; }
    ...
    // the desired result
    realm::util::UniqueFunction<void(const realm::app::Response&)> network_callback;
    std::shared_ptr<realm::app::GenericNetworkTransport> transport = std::make_shared<HookedTransport>(&network_callback);
