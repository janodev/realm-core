...
#include <realm/sync/history.cpp>
...
private:
    ...
    size_t write_event(Timestamp timestamp, StringData activity, StringData event_type, StringData data, bool = true)
    {
...
class AuditRealmFactory : public std::enable_shared_from_this<AuditRealmFactory> {
public:
    using ErrorHandler = std::function<SyncSessionErrorHandler>;

    static std::shared_ptr<AuditRealmFactory> get_factory(std::shared_ptr<SyncUser> user,
    ...
    AuditRealmFactory(std::shared_ptr<SyncUser> user, std::string const& partition_prefix, ErrorHandler error_handler,
                      util::Logger& logger);
...
std::shared_ptr<AuditRealmFactory> AuditRealmFactory::get_factory(std::shared_ptr<SyncUser> user,
                                                                  std::string const& partition_prefix,
                                                                  util::Logger& logger, ErrorHandler error_handler)
{
    // FIXME: clean up expired
    static std::mutex s_factory_mutex;
    static std::unordered_map<std::string, std::weak_ptr<AuditRealmFactory>> s_factories;
    std::lock_guard lock{s_factory_mutex};
    auto key = util::format("%1-%2", user->identity(), partition_prefix); // appId too?
    auto& weak_factory = s_factories[key];
    if (auto factory = weak_factory.lock()) {
        return factory;
    }

    auto factory = std::make_shared<AuditRealmFactory>(user, partition_prefix, error_handler, logger);
    factory->scan_for_realms_to_upload();
    weak_factory = factory;
    return factory;
}

AuditRealmFactory::AuditRealmFactory(std::shared_ptr<SyncUser> user, std::string const& partition_prefix,
                                     ErrorHandler error_handler, util::Logger& logger)
    : m_user(user)
    ...
    , m_path_root([&] {
        ...
        return util::format("%2%1realm-audit%1%3%1%4%1%5%1", separator, base_file_path,
                            m_user->sync_manager()->app().lock()->config().app_id, m_user->identity(),
                            partition_prefix);
    }())
    , m_logger(logger)
{
...
void AuditRealmFactory::write(util::FunctionRef<void(Transaction&)> func)
{
    if (m_current_realm) {
...
void AuditRealmFactory::wait_for_upload(std::shared_ptr<SyncSession> session)
{
...
void AuditRealmFactory::scan_for_realms_to_upload()
{
    util::CheckedLockGuard lock(m_mutex);
    ...
    while (dir.next(file_name)) {
        ...
        if (!StringData(file_name).ends_with(".realm"))
            continue;
        auto db = DB::create(sync::make_client_replication(), path);
        ...
        wait_for_upload(m_user->sync_manager()->get_session(
            db, SyncConfig{m_user, util::format("%1-%2", m_partition_prefix, partition)}));
        return;
    }

    // Did not find any files needing to be uploaded
    m_cv.notify_all();
}

void AuditRealmFactory::open_new_realm()
{
    ...
    auto sync_config = std::make_shared<SyncConfig>(m_user, '"' + m_partition_prefix + "-" + partition + '"');
    sync_config->client_resync_mode = ClientResyncMode::Manual;
    sync_config->recovery_directory = std::string("io.realm.audit");
    sync_config->error_handler = m_error_handler;
    ...
    try {
        ...
        m_logger.error("Audit: Error opening audit Realm: %1", e.what());
        abort(); // FIXME
        // do something
    }
...
void AuditRealmFactory::wait_for_uploads()
{
...
public:
    AuditContext(std::shared_ptr<DB> source_db, AuditConfig const& audit_config);
    ~AuditContext();

    void init(std::weak_ptr<AuditContext>, RealmConfig const& parent_config);
...
private:
    ...
    std::shared_ptr<AuditRealmFactory> m_realm_factory;
...
AuditContext::AuditContext(std::shared_ptr<DB> source_db, AuditConfig const& audit_config)
...
void AuditContext::init(std::weak_ptr<AuditContext> weak_self, RealmConfig const& parent_config)
{
    m_weak_self = weak_self;
    REALM_ASSERT(parent_config.audit_config);
    REALM_ASSERT(parent_config.sync_config);

    auto& audit_config = *parent_config.audit_config;
    ...
    m_realm_factory =
        AuditRealmFactory::get_factory(audit_user, audit_config.partition_value_prefix, *m_logger,
                                       [weak_self](std::shared_ptr<SyncSession> const&, SyncError error) {
...
void AuditContext::process_scope(AuditContext::Scope& scope) const
{
    ...
    try {
        ...
        m_realm_factory->write([&](Transaction& tr) {
            ...
            // Larger batch sizes improve compressibility and reduce the total
            // runtime, but we have to stay under 16 MB or the changeset will
            // be rejected by the server.
            // FIXME: actually test what the typical expansion from replication logs is
            constexpr const size_t max_batch_size = 20'000;
            constexpr const size_t max_payload_size = 8 * 1024 * 1024;
...
{
    m_realm_factory->wait_for_uploads();
}
...
std::shared_ptr<AuditInterface> make_audit_context(std::shared_ptr<DB> db, RealmConfig const& config)
{
    ...
    auto context = std::make_shared<AuditContext>(db, audit_config);
