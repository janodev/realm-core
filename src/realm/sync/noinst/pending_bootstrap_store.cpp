...
PendingBootstrapStore::PendingBatch PendingBootstrapStore::get_next()
{
    ...
    for (size_t idx = 0; idx < changeset_list.size(); ++idx) {
        ...
        batch.changeset_data.emplace_back(cur_changeset_obj.get<BinaryData>(m_changeset_data));

        Transformer::RemoteChangeset cur_changeset;
        cur_changeset.data = batch.changeset_data.back().get();
        ...
        batch.changesets.push_back(cur_changeset);
    }
